#!/usr/bin/env python3
"""
match_db.py v4 – Wikidata matching
...
def fetch_wikidata(sparql, label):
    ...
    # Primární index: norm(název) → data
    by_norm = {}
    # Sekundární index: délka normu → list klíčů (pro fuzzy)
...
def fuzzy_match(title_norm, by_norm, by_len, year=None, threshold=0.85):
    """
    Rychlý fuzzy matching:
    1. Přímá shoda
    2. Prohledá pouze záznamy s podobnou délkou názvu (±3 znaky)
    """
    # Přímá shoda
    ...
    for k in candidates:
        if year:
            val = by_norm[k]
            if val.get('year') and abs(val['year'] - year) > 3:
                continue
...
def match_records(records, by_norm, by_len, title_field, threshold, label):
    ...
    for i, r in enumerate(records):
        ...
        year = r.get('year')
        title_n = norm(title)
        hit = fuzzy_match(title_n, by_norm, by_len, year=year, threshold=threshold)
        if hit:
...
def main():
    print('=== Wikidata Matching v4 ===', flush=True)
    ...
    if wd_movies:
        movies = match_records(movies, wd_movies, wd_movies_len,
                               title_field='clean_title', threshold=0.85, label='filmy')
    else:
        print('WARN: Wikidata filmy nedostupne – preskakuji.')

    if wd_series:
        series = match_records(series, wd_series, wd_series_len,
                               title_field='show_title', threshold=0.82, label='serialy')
    else:
