...
JUNK = re.compile(
    ...
    r'|2160p|4k\b|uhd\b|bdremux\b'
    r'|1080[pi]|720p|576p|480p|fullhd\b'
    r'|blu.?ray|web.?rip|web.?dl|hd.?rip|bdrip|dvdrip|hdtv'
...
def _cz(n):
    nl = n.lower()
    return any(x in nl for x in ['.cz.','_cz_','-cz-',' cz ','czech','cesky',
...
def fetch_query(query, token, max_pages=3):
    results = []
...
def movie_queries():
    ...
    for y in range(YEAR, YEAR-5, -1):
        q += [f'{y}', f'1080p {y}', f'cz dabing {y}', f'4k {y}', f'czech {y}', f'bluray {y}']
    for y in range(YEAR-5, 2009, -1):
        q.append(f'{y}')
        q.append(f'cz dabing {y}')
    q += ['4k bluray', '1080p bluray cz', 'cz dabing', 'sk dabing', 'webrip cz', '4k hdr', 'uhd bluray']
    return list(dict.fromkeys(q))
...
def main():
    ...
    for i, q in enumerate(sq):
        for r in fetch_query(q, token, max_pages=3):
            ...
            if p and p['type'] == 'series':
                series_raw.append(p)
        if i % 10 == 0:
            print(f'  {i}/{len(sq)} dotazů – {len(series_raw)} záznamů')
        time.sleep(0.15)
    ...
    for i, q in enumerate(mq):
        for r in fetch_query(q, token, max_pages=3):
            ...
            if p and p['type'] == 'movie':
                movies_raw.append(p)
        if i % 10 == 0:
            print(f'  {i}/{len(mq)} dotazů – {len(movies_raw)} záznamů')
        time.sleep(0.15)
